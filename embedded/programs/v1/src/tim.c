...
void TIM_init(TIM_TypeDef* tim, uint16_t prescaler, uint32_t limit, uint8_t update_interrupt) {
	if ((((uint32_t)tim) - APB1PERIPH_BASE) >= 0x00010000UL)	{ RCC->APB2ENR |= (1 << (((uint32_t)(tim - APB2PERIPH_BASE) >> 10u) & 0xfu)); }
	else														{ RCC->APB1ENR |= (1 << (((uint32_t)(tim - AHB1PERIPH_BASE) >> 10u) & 0xfu)); }
	//RCC_TypeDef* ptr = RCC;
	tim->PSC = prescaler;
	tim->ARR = limit;
