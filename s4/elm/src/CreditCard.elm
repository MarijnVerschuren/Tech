...
toDigitsRev x = List.map (\c -> Maybe.withDefault 0 (String.toInt c)) (List.foldl (::) [] (String.split "" x))

doubleSecond: List Int -> List Int
doubleSecond xs = List.indexedMap (\i -> \d -> d * (2 - (modBy 2 i))) xs

sumDigits: List Int -> Int
sumDigits xs = List.foldl (+) 0 xs
...
check_number num = (
    modBy 10 (                                                                          -- calculate sum % 10
        List.foldl (+) 0 (                                                              -- add all digits
            List.indexedMap
                (\i -> \d -> Maybe.withDefault 0 (String.toInt d) * (2 - (modBy 2 i)))  -- double every second digit
                (List.foldl (::) [] (String.split "" (String.fromInt num)))             -- reverse list of digits
            )
        )
    ) == 0                                                                              -- check if the sum evenly divisible by 0

check_list: List Int -> Int
check_list numbers = List.length (List.filter (\i -> check_number i) numbers)
-- </own implementation>
