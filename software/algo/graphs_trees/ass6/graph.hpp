...
private:
	...
	static std::vector<type> checked;  // this vector holds all checked nodes when using recursion
	type data;
public:
	...
	Graph<type>* depth(uint64_t* const depth) const {
		...
		this->checked.clear();  // clear checked array
		(*depth) = 0; uint64_t child_depth = 0;
		for (auto child : this->children) {
			if (std::find(this->checked.begin(), this->checked.end(), child->data) == this->checked.end()) {
		...
		this->checked.push_back(this->data);
