...
template<typename type> Path<type>* BFS(Graph<type>*, type, Path<type>* = nullptr, Graph<type>* = nullptr);
...
public:
	...
	/*Graph<type>* depth(uint64_t* const depth) {
	...
	friend Path<type>* BFS<type>(Graph<type>*, type, Path<type>*, Graph<type>*);
};
...
template<typename type>
Path<type>* BFS(Graph<type>* graph, type data, Path<type>* path, Graph<type>* parent) {
	if (!graph) { return nullptr; }
	bool root = !path; if (root) { path = new Path<type>(graph); }
	if (graph->data == data) { return path; }
	/*if (parent) {
